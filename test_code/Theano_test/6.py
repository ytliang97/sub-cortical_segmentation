...
def minmax_normalization(data): # 0 ~ 1
...
#cost = T.mean(T.square(l2.outputs - y)) # could overfitting
cost = T.mean(T.square(l2.outputs - y)) + 0.1*( (l1.W**2).sum()+(l2.W**2).sum() ) #l2 regularization: add a number to avoid overfitting, less than 1 is better
...
with open('save/model.pickle', 'wb') as file:
    model = [l1.W.get_value(), l1.b.get_value(), l2.W.get_value(), l2.b.get_value()]
...
with open('save/model.pickle', 'rb') as file:
    ...
    l1.W.set_value(model[0])
    l1.b.set_value(model[1])
    l2.W.set_value(model[2])
    l2.b.set_value(model[3])
    print(l1.b.get_value()[:10])
